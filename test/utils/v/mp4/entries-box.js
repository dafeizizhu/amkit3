...
describe('utils/v/mp4/entries-box', () => {
  ...
  it(':constructor', () => {
    assert.throws(() => new EntriesBox({ type }))
    assert.throws(() => new EntriesBox({ type, entries: 'str' }))
    assert.throws(() => new EntriesBox({ type, entries: [1, 2, 3] }))
    assert.doesNotThrow(() => {
      var box = new EntriesBox(o)
  ...
  it(':fromStream', () => {
    var box = EntriesBox.fromStream(new DataStream(boxData.buffer))
  ...
  it(':dataFromStream', () => {
    var box = EntriesBox.dataFromStream(type, boxData.byteLength, new DataStream(data.buffer))
    assert.equal(box.type, type)
    assert.equal(box.version, version)
    assert.deepEqual(box.flags, flags)
    assert.deepEqual(box.entries, entries)
